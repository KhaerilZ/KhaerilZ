...
const isOwner = ownerNumber == sender ? true : ["6281242648600@s.whatsapp.net","6281242648600@s.whatsapp.net","6281242648600@s.whatsapp.net"].includes(sender) ? true : false
...
case 'apakahhamkagay':
if (cekUser("id", sender) == null) return reply("Kamu belum terdaftar di database bot, Silahkan daftar terlebih dahulu, gunakan command #daftar")
reply("*Pertanyaan :* Apakah hamka gay?\n\n*Jawaban :* Ya")
break
...
reply("Sukses")
break
case 'jelek': case 'cantik': case 'ganteng': case 'sad': case 'haram': case 'halal': case 'bego': case 'anjing': case 'biadab': case 'ramah': case 'satir': case 'manis': case 'pahit': case 'hitam': case 'rasis': case 'baik': case 'jahat': case 'kaya': case 'miskin': case 'pintar': case 'bodoh': case 'imut': case 'kocak': case 'kadang': case 'galau': case 'sange':  
if (cekUser("id", sender) == null) return reply("Kamu belum terdaftar di database bot, Silahkan daftar terlebih dahulu, gunakan command #daftar")
...
const x28 = ["Iya", "Tidak", "mungkin"][Math.floor(Math.random() * (["Iya", "Tidak", "mungkin"].length))]
reply(`*Pertanyaan* : ${q}\n\n*Jawaban* : ${x28}`)
break 
...
if (!q) return reply(`Kirim gambar/vidio dengan caption ${command} atau balas gambar/vidio yang sudah dikirim\nNote : Maximal vidio 5 detik!`)
}
break 
case 'report':
if (cekUser("id", sender) == null) return reply("Kamu belum terdaftar di database bot, Silahkan daftar terlebih dahulu, gunakan command #daftar")
...
if (!isGroupAdmins) return reply("ONLY ADMIN");
if (!isBotGroupAdmins) return reply("BOT BUKAN ADMIN");
if (!q) return reply("*Masukan nomor yang ingin ditambahkan di group*\n\n_Contoh : .add 62881xxxxxxx_")const topnumberor = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
...
if (!isGroupAdmins) return reply("ONLY ADMIN");
if (!isBotGroupAdmins) return reply("BOT BUKAN ADMIN");
if (nay.message.extendedTextMessage === undefined || nay.message.extendedTextMessage === null) return reply("*Tag orang yang anda ingin keluarkan dari group ini*\n\n_Contoh : @user_")
